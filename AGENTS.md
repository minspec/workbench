...
- **Mate** (`vendor/bin/mate`, the CLI tool surface serving this application's measured truth) is per-checkout: after `git worktree add`, run `composer install` before Mate tools exist in that worktree. Composer's global cache makes this cheap. The `symfony/ai-mate-composer-plugin` re-runs extension discovery on every install, which also reconciles the generated agent skills and the marked block Mate maintains at the end of this file.
- Mate 0.13 is not an MCP server: its tools run as `vendor/bin/mate tools:call <tool>` (see `mate/AGENT_INSTRUCTIONS.md`), and it installs agent skills into `.claude/skills/` and `.agents/skills/` — tracked, so the tool surface agents see is reviewable. `.mcp.json` therefore registers Serena only.
